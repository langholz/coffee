...
The dataset was [acquired](#retrieve_data) using CQI's undocumented API, [normalized](#normalize_data) and then [saved](#normalize_data). At the time of obtaining this data there were only 3 robusta coffee records. Therefore, the focus was determined to fall fully on arabica coffees. In general the data is divided into three different areas: coffee information, grades/cupping scores and green analysis.
...
The coffee information dataset has several issues which were addressed using OpenRefine[^5]. The operation history has been stored as JSON files (`*_recipe.json`) under the `data/openrefine` directory.
...
Using the clean data a SQLite database was [setup and opened](#setup_and_open_database) and each data subset [written to](#write_data_and_list_tables) a corresponding table with the same name.
...
kable(result) %>% kable_styling(position = "center")
```

This could mean that the cupping score/grading was finished before the entry was created.
...
In this case the manually calculated total amounts (`one_total` and `two_total`) are double ($2x$) the ones reported by the data set. This could be caused by a either an ETL process gone wrong or an incorrect data entry.
...
As in the case before for the same entry, we see that the manually calculated defect amounts (`one_defects` and `two_defects`) are larger by two times plus one ($2x + 1$) the reported value by the data set. This could be caused by a either an ETL process gone wrong or an incorrect data entry.
...
In this case we are missing an entry in both. It could mean that it still has not been added and/or reviewed. This entry would likely be removed from the data set.
...
A [retrieve and clean workflow](#retrieve_and_clean_workflow) was designed using YesWorkflow[^6] and is provided as supplemental information as `workflow/coffee.yw`. Following are multiple visualization levels of it.
