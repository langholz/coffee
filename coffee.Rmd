...
```{r echo=FALSE, ref.label="calculate_csv_diff_summaries", eval=TRUE, warning=FALSE}
```

Using `csvdiff`[^8], the changes performed in total and at each stage can be calculated per each file by the amount rows that were either removed, added and/or changed.

#### By type and stages

The changes by type and stages can be [summarized](#calculate_csv_diff_summaries) with the following tables.

##### Coffee information

```{r coffee_information_results_summary, echo=FALSE}
kable(summaries[[1]]) %>% kable_styling(position = "center") %>% row_spec(0, bold = TRUE)
```

##### Grades/Cupping scores

```{r grading_cupping_scores_results_summary, echo=FALSE}
kable(summaries[[2]]) %>% kable_styling(position = "center") %>% row_spec(0, bold = TRUE)
```

##### Green analysis

```{r green_analysis_results_summary, echo=FALSE}
kable(summaries[[3]]) %>% kable_styling(position = "center") %>% row_spec(0, bold = TRUE)
```

#### Totals

```{r echo=FALSE, ref.label="calculate_total_csv_diff_summaries", eval=TRUE, warning=FALSE}
```

The total changes between when the data was first downloaded and the final data cleaning changes can then be [summarized](#calculate_total_csv_diff_summaries) as follows:

```{r total_results_summary, echo=FALSE}
kable(total_summaries_df) %>% kable_styling(position = "center") %>% row_spec(0, bold = TRUE)
```
...
### Calculate CSV difference summaries {#calculate_csv_diff_summaries}

```{r calculate_csv_diff_summaries, echo=TRUE, eval=FALSE}
...
### Calculate total CSV difference summaries {#calculate_total_csv_diff_summaries}

```{r calculate_total_csv_diff_summaries, echo=TRUE, eval=FALSE}
...
total_summaries_df <- do.call(rbind, total_summaries)
rownames(total_summaries_df) <- c(
  "Coffee information",
  "Grades/Cupping scores",
  "Green analysis")
```
